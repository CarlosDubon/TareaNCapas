		...
		<div class="d-flex align-items-center flex-column mt-3"">
			...
			<div class="d-flex flex-wrap w-75 m-3 justify-content-around" >
				<c:forEach items="${sucursales}" var="s">
					<div class="card text-white bg-primary w-25 m-3" onClick="findSucursal(${s.id})" style="cursor:pointer">
					  <div class="card-header d-flex justify-content-between">
					  	${s.nameSupport}
					  	<div>					
						  	<i class="fas fa-cog mr-3"></i>
						  	<i class="fas fa-times" onClick="handleDelete(${s.id})"></i>
					  	</div>
					  </div>
					  <div class="card-body">
					    <h4 class="card-title">${s.name}</h4>
					    <p class="card-text">Todos los dias de: ${s.openS} -  ${s.closeS} </p>
					  </div>
					</div>
				</c:forEach>
			</div>
		</div>
		<a class="btn btn-success floating-btn" data-toggle="modal" data-target="#new-sucursal-modal">
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	      	<form:form action="${pageContext.request.contextPath}/addSucursal" method="POST" modelAttribute="sucursal" >
